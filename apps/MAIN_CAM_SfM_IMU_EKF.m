...
toc
%% Real Data: Visual Monocular Odometry Camera poses Loading Using Structure from Motion Algorithm 
tic
% The flag useVO determines if visual odometry is used:
useVO = true; % Both IMU and visual odometry from SfM Algorithm are used.
...
toc
%% Real Data: VISUAL MONOCULAR ODOMETRY CAMERA Poses Computation Using Structure from Motion Algorithm
tic
% The flag useVO determines if visual odometry is used:
useVO = true; % Both IMU and visual odometry from SfM Algorithm are used.
...
KF = numel(imds.Files);  % (All frames for Best Performance)
images = cell(1, KF);
k = 1;
prev_stop  = 0;
