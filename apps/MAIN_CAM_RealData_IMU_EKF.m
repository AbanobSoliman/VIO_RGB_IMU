...
% Developed by Abanob Soliman (abanob.soliman@univ-evry.fr)
% for Master's Thesis Under Supervision of 
% Dr. Fabien Bonardi (fabien.bonardi@univ-evry.fr) 
% Dr. Hicham HADJ-ABDELKADER (hicham.hadjabdelkader@univ-evry.fr)
% Prof. Dr. Samia Bouchafa-Bruneau (samia.bouchafabruneau@univ-evry.fr)
...
toc
%% Real Data: Inserting Motion Capture System Ground Truth Data
...
toc
%% Real Data: VISUAL MONOCULAR ODOMETRY CAMERA Poses Computation Using Synthetic Data
...
useVO = true;  % Both IMU and visual odometry from Synthetic data are used.

load('pqfile2.mat','camPoses_Transl','camPoses','xyzPoints','xyzPoints1','vSet','reprojectionErrors1','reprojectionErrors2')

paramsVO.scale = 0.8;
paramsVO.sigmaN = 0.00139;
paramsVO.tau = 232;
paramsVO.sigmaB = sqrt(0.00134);
...
Rang = [0.1128,0.1185,0.1152]; % CAM Angle    measurement noise (vision-cam)
Rpos = [9.59e-8,8.98e-8,0.00000011568]; % CAM Position measurement noise (vision-cam)
dR   = [Rpos,Rang];
...
while imuRead
    ...
    [States,qw_i] = IMU_Propagation_(States,T,accel.curr,gyro,qw_i,ref);   % Paper IMU States Propagation Formulas -Recommended- 
    ...
    if ff == k_imu && useVO 
        ...
        % Map Loss 
        if MapLOSS 
            if (ff >= imuSteps/2 && ff < imuSteps/1.5)
                camPoses_Transl(k_cam,:) = camPoses_Transl(k_cam-1,:);
                camPoses_Orient(k_cam,:) = camPoses_Orient(k_cam-1,:);
                test_estimated(k_cam,1)  = test_estimated(k_cam-1,1);
                test_pestimated(k_cam,:) = test_pestimated(k_cam-1,:);
                ...
                test_pv_w(ff,:) = fusionfilt.State(25:27)';
                ...
                disp("Camera map is lost at this step!");
                k_cam = k_cam + 1;
                k_fp  = k_fp + 1;
                if k_cam > camSteps
                    camRead = false;
        ...
        % Drop some Camera readings in between frames
        if MapDROP && (k_map == drop)
            camPoses_Transl(k_cam,:) = camPoses_Transl(k_cam-1,:);
            camPoses_Orient(k_cam,:) = camPoses_Orient(k_cam-1,:);
            test_estimated(k_cam,1)  = test_estimated(k_cam-1,1);
            test_pestimated(k_cam,:) = test_pestimated(k_cam-1,:);
            ...
            disp("Camera map is dropped at this step!");
            k_map = 1;
            k_cam = k_cam + 1;
            k_fp  = k_fp + 1;
            if k_cam > camSteps
...
figure
subplot(2,1,1)
plot(fuseTimeSteps(1:length(test_pv_w)),test_pv_w(:,1),fuseTimeSteps(1:length(test_pv_w)),test_pv_w(:,2),fuseTimeSteps(1:length(test_pv_w)),test_pv_w(:,3))
grid on
xlabel('Fused IMU & CAM Instances (sec.)')
ylabel('position drift p_v^w [m]')
title('X, Y, Z representation of the calculated position between the black-box visual frame and the world frame')
legend('P_v^w X','P_v^w Y','P_v^w Z')
subplot(2,1,2)
...
figure
plot(camTimeSteps,test_pestimated(:,1),camTimeSteps(1:length(camPoses_Transl)),camPoses_Transl(:,1),fuseTimeSteps,pqpos(1:length(fuseTimeSteps),1),gtTimeSteps,gtPositions(:,1))
grid on
xlabel('Fused IMU & CAM Instances (sec.)')
ylabel('X (m)')
...
figure
plot(camTimeSteps,test_pestimated(:,2),camTimeSteps(1:length(camPoses_Transl)),camPoses_Transl(:,2),fuseTimeSteps,pqpos(1:length(fuseTimeSteps),2),gtTimeSteps,gtPositions(:,2))
grid on
xlabel('Fused IMU & CAM Instances (sec.)')
ylabel('Y (m)')
...
figure
plot(camTimeSteps,test_pestimated(:,3),camTimeSteps(1:length(camPoses_Transl)),camPoses_Transl(:,3),fuseTimeSteps,pqpos(1:length(fuseTimeSteps),3),gtTimeSteps,gtPositions(:,3))
grid on
xlabel('Fused IMU & CAM Instances (sec.)')
ylabel('Z (m)')
...
figure
plot(camTimeSteps,test_orient(:,1),camTimeSteps(1:length(camPoses_Orient)),camPoses_Orient(:,1),fuseTimeSteps,euler_orient(1:length(fuseTimeSteps),1),gtTimeSteps,gtOrientations(:,1))
grid on
xlabel('Fused IMU & CAM Instances (sec.)')
ylabel('\phi°')
...
figure
plot(camTimeSteps,test_orient(:,2),camTimeSteps(1:length(camPoses_Orient)),camPoses_Orient(:,2),fuseTimeSteps,euler_orient(1:length(fuseTimeSteps),2),gtTimeSteps,gtOrientations(:,2))
grid on
xlabel('Fused IMU & CAM Instances (sec.)')
ylabel('\theta°')
...
figure
plot(camTimeSteps,test_orient(:,3),camTimeSteps(1:length(camPoses_Orient)),camPoses_Orient(:,3),fuseTimeSteps,euler_orient(1:length(fuseTimeSteps),3),gtTimeSteps,gtOrientations(:,3))
grid on
xlabel('Fused IMU & CAM Instances (sec.)')
ylabel('\psi°')
...
OKVISVelocities    = [Tbl_OKVIS.v_x,Tbl_OKVIS.v_y,Tbl_OKVIS.v_z];
...
figure
plot(gtTimeSteps,gtOrientations(:,1),camTimeSteps(1:length(camPoses_Orient)),camPoses_Orient(:,1))
grid on
...
figure
plot(gtTimeSteps,gtOrientations(:,2),camTimeSteps(1:length(camPoses_Orient)),camPoses_Orient(:,2))
grid on
xlabel('GT & CAM Instances (sec.)')
ylabel('\theta°')
...
figure
plot(gtTimeSteps,gtOrientations(:,3),camTimeSteps(1:length(camPoses_Orient)),camPoses_Orient(:,3))
grid on
xlabel('GT & CAM Instances (sec.)')
ylabel('\psi°')
...
figure
plot(gtTimeSteps,gtPositions(:,1),camTimeSteps(1:length(camPoses_Transl)),camPoses_Transl(:,1))
grid on
xlabel('GT & CAM Instances (sec.)')
ylabel('X (m)')
...
figure
plot(gtTimeSteps,gtPositions(:,2),camTimeSteps(1:length(camPoses_Transl)),camPoses_Transl(:,2))
grid on
xlabel('GT & CAM Instances (sec.)')
ylabel('Y (m)')
...
figure
plot(gtTimeSteps,gtPositions(:,3),camTimeSteps(1:length(camPoses_Transl)),camPoses_Transl(:,3))
grid on
xlabel('GT & CAM Instances (sec.)')
ylabel('Z (m)')
...
figure
subplot(3,1,1)
plot(fuseTimeSteps,pqpos(1:length(fuseTimeSteps),1),gtTimeSteps,gtPositions(:,1),camTimeSteps(1:length(camPoses_Transl)),camPoses_Transl(:,1))
grid on
ylabel('X (m)')
title('Fused IMU & CAM position with Ground-Truth')
subplot(3,1,2)
plot(fuseTimeSteps,pqpos(1:length(fuseTimeSteps),2),gtTimeSteps,gtPositions(:,2),camTimeSteps(1:length(camPoses_Transl)),camPoses_Transl(:,2))
grid on
ylabel('Y (m)')
subplot(3,1,3)
plot(fuseTimeSteps,pqpos(1:length(fuseTimeSteps),3),gtTimeSteps,gtPositions(:,3),camTimeSteps(1:length(camPoses_Transl)),camPoses_Transl(:,3))
grid on
xlabel('Fused IMU & CAM Instances (sec.)')
ylabel('Z (m)')
legend('EKF-Fusion','Ground-Truth','CAM-10 KF')

% plotting and comparing the Fused, GT & OKVIS (xyz-velocities) 
figure
subplot(3,1,1)
plot(fuseTimeSteps,pqvel(1:length(fuseTimeSteps),1),camTimeSteps(1:length(OKVISVelocities)),OKVISVelocities(:,1))
grid on
ylabel('V_x (m / sec.)')
subplot(3,1,2)
plot(fuseTimeSteps,pqvel(1:length(fuseTimeSteps),2),camTimeSteps(1:length(OKVISVelocities)),OKVISVelocities(:,2))
grid on
ylabel('V_y (m / sec.)')
subplot(3,1,3)
plot(fuseTimeSteps,pqvel(1:length(fuseTimeSteps),3),camTimeSteps(1:length(OKVISVelocities)),OKVISVelocities(:,3))
grid on
xlabel('Time (sec.)')
ylabel('V_z (m / sec.)')
legend('Our EKF','OKVIS')

% plotting and comparing the Fusion & Ground-Truth (Orientations) 
figure
subplot(3,1,1)
plot(camTimeSteps(1:length(OKVISOrientations)),OKVISOrientations(:,1),gtTimeSteps,gtOrientations(:,1),fuseTimeSteps,euler_orient(1:length(fuseTimeSteps),1));
grid on
ylabel('\phi°')
subplot(3,1,2)
plot(camTimeSteps(1:length(OKVISOrientations)),OKVISOrientations(:,2),fuseTimeSteps,euler_orient(1:length(fuseTimeSteps),2));
grid on
ylabel('\theta°')
subplot(3,1,3)
plot(camTimeSteps(1:length(OKVISOrientations)),OKVISOrientations(:,3),fuseTimeSteps,euler_orient(1:length(fuseTimeSteps),3));
grid on
xlabel('Time (sec.)')
ylabel('\psi°')
legend('OKVIS','Ground-Truth','Our EKF')

% plotting and comparing the CAM, GT & Fused (phi-orientation-Roll)
figure
plot(fuseTimeSteps,euler_orient(1:length(fuseTimeSteps),1),gtTimeSteps,gtOrientations(:,1),camTimeSteps(1:length(camPoses_Orient)),camPoses_Orient(:,1))
grid on
xlabel('Fused IMU & CAM Instances (sec.)')
ylabel('\phi°')
...
figure
plot(fuseTimeSteps,euler_orient(1:length(fuseTimeSteps),2),gtTimeSteps,gtOrientations(:,2),camTimeSteps(1:length(camPoses_Orient)),camPoses_Orient(:,2))
grid on
xlabel('Fused IMU & CAM Instances (sec.)')
ylabel('\theta°')
...
figure
plot(fuseTimeSteps,euler_orient(1:length(fuseTimeSteps),3),gtTimeSteps,gtOrientations(:,3),camTimeSteps(1:length(camPoses_Orient)),camPoses_Orient(:,3))
grid on
xlabel('Fused IMU & CAM Instances (sec.)')
ylabel('\psi°')
...
grid on
ylabel('Error (m)')
title('Fused IMU & CAM Errors Analysis')
...
% (biases - Visual Odometry scale - Visual Drifts)
figure
subplot(6,1,1)
...
grid on
title('(a) Biases & Visual Odometry Scale Factor')
ylabel('b_a')
legend('b_a^x','b_a^y','b_a^z')
subplot(6,1,2)
...
grid on
ylabel('b_w')
legend('b_w^\phi','b_w^\theta','b_w^\psi')
subplot(6,1,3)
plot(fuseTimeSteps,lmda(1:length(fuseTimeSteps),1))
grid on
ylabel('\lambda')
subplot(6,1,4)
plot(camTimeSteps,test_qvwDeg(:,1))
grid on
title('(b) Visual Drifts [deg]')
ylabel('q_v^w [\phi]')
subplot(6,1,5)
plot(camTimeSteps,test_qvwDeg(:,2))
grid on
ylabel('q_v^w [\theta]')
subplot(6,1,6)
plot(camTimeSteps,test_qvwDeg(:,3))
grid on
xlabel('Time (sec.)')
ylabel('q_v^w [\psi]')
...
% Plotting the gaussian distribution contours (confidence intervals) 
error_ellipse(pqpos);
toc
%% Real Data: Display the refined camera poses and 3-D world points from Structure from Motion Algorithm
tic
...
% Display 3D paths
Plot1=plot3(pqpos(1:length(fuseTimeSteps),1),pqpos(1:length(fuseTimeSteps),2),pqpos(1:length(fuseTimeSteps),3));
hold all
Plot2=plot3(OKVISPositions(:,1),OKVISPositions(:,2),OKVISPositions(:,3));
hold all
Plot3=plot3(gtPositions(:,1),gtPositions(:,2),gtPositions(:,3));
hold all
legend([Plot1 Plot2 Plot3],{'EKF-Fusion','OKVIS','Ground-Truth'});

% Display the dense 3-D world points.
xyzPointsN(:, 1) = xyzPoints(:, 1)/7;
xyzPointsN(:, 2) = xyzPoints(:, 2)/7;
xyzPointsN(:, 3) = xyzPoints(:, 3)/7;
pcshow(xyzPointsN(:, :), 'VerticalAxis', 'y', 'VerticalAxisDir', 'down', ...
    'MarkerSize', 80);
grid on
hold off

% Specify the viewing volume.
loc1 = camPosess.Location{1}/30;
xlim([loc1(1)-.7, loc1(1)+.7]);
ylim([loc1(2)-.7, loc1(2)+.7]);
zlim([loc1(3)-.7, loc1(3)+2.5]);
camorbit(0, -30);
