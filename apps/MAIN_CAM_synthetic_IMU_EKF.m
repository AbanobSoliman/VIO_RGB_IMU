...
toc
%% MATLAB Synthetic data for Ground Truth + IMU + CAM
tic
load('Propagation_syn.mat','imuSamplesPerCamera','numIMUSamples','numSecondsToSimulate');
ref = 'ENU';
% MapDROP = true;
drop    = 7;
MapDROP = false;
...
% Ground Truth
load('GT_syn.mat','pos','orient');
gtSteps        = size(pos,1);
gtQuaternions  = orient;
gtOrientations = quat2eul(orient,'ZYX')*180/pi; % In Degrees
gtPositions    = pos;
gtTimeSteps    = linspace(1,numSecondsToSimulate,numIMUSamples);

% Monocular Vision Camera Calibration Parameters 
useVO           = true;  % Both IMU and visual odometry from Synthetic data are used.
% useVO           = false;  % IMU Synthetic data only is used.
...
camSteps        = numIMUSamples/imuSamplesPerCamera;
camTimeSteps    = linspace(1,numSecondsToSimulate,camSteps);

% IMU ( accel. + Gyro ) 
load('IMU_syn.mat','accelMeas','gyroMeas','angvel','acc');
imuA.data.gyro  = angvel;
imuA.data.accel = [acc(:,1:2),-accelMeas(:,3)];
imuSteps = numIMUSamples;
imuA.data.time  = linspace(1,numSecondsToSimulate,imuSteps);
T = imuA.data.time(2)-imuA.data.time(1);
...
accelerometer_random_walk   =  (0.0001*T)*ones(1,3);   % sigma_nba->[m/s^3/sqrt(Hz)](accel bias diffusion)
accelerometer_noise_density =  (0.0001/T)*ones(1,3);    % sigma_na->[m/s^2/sqrt(Hz)](accel "white noise")
gyroscope_random_walk       =  (1e-09*T)*ones(1,3);   % sigma_nbw->[rad/s^2/sqrt(Hz)](gyro bias diffusion)
gyroscope_noise_density     =  (1e-06/T)*ones(1,3);   % sigma_nw->[rad/s/sqrt(Hz)](gyro "white noise")

% Estimation Comparison To MATLAB Algorithms
load('Est_syn.mat','posEst','orientEst','velEst');
Rang = [0,9.59e-7,9.59e-7]; % CAM Angle    measurement noise (vision-cam)
Rpos = [0,0,0]; % CAM Position measurement noise (vision-cam)
dR   = [Rpos,Rang];
R = diag(dR);
...
P(1:3,1:3)  = diag([1e-5,1e-5,1e-8]); P(4:6,4:6)  = 1e-5; P(7:9,7:9)  = 10^-5;
P(10:12,10:12)   = 10^-7;
P(13:15,13:15)   = 1e-7;
P(16,16) = 10^-7;
P(17:19,17:19) = 10^-7; P(20:22,20:22) = 10^-7;
P(23:25,23:25) = 10^-7; P(26:28,26:28) = 10^-10;
...
error_ellipse(pqpos);
toc
%% Plot for comparison with the MATLAB Synthetic data Results
...
if useVO
    plot3(gtPositions(:,1), gtPositions(:,2), gtPositions(:,3), '-.', ...
        camPoses_Transl(:,1), camPoses_Transl(:,2), camPoses_Transl(:,3), ...
        pqpos(:,1), pqpos(:,2), pqpos(:,3), ...
        'LineWidth', 3)
